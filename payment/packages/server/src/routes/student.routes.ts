...
studentRouter.get("/export", rbacMiddleware(Role.ADMIN, Role.TEACHER), (req, res, next) => studentController.exportExcel(req, res, next));
studentRouter.get("/:id", rbacMiddleware(Role.ADMIN, Role.TEACHER), (req, res, next) => studentController.getById(req, res, next));
studentRouter.post("/", rbacMiddleware(Role.ADMIN, Role.TEACHER), (req, res, next) => studentController.create(req, res, next));
studentRouter.put("/:id", rbacMiddleware(Role.ADMIN, Role.TEACHER), (req, res, next) => studentController.update(req, res, next));
studentRouter.delete("/:id", rbacMiddleware(Role.ADMIN, Role.TEACHER), (req, res, next) => studentController.delete(req, res, next));
